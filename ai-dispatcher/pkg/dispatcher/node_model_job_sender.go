...
func (sender *nodeModelJobSender) getLastMIdPrediction(datahubServiceClnt *datahubpkg.Client,
	...
	for _, metricType := range []datahub_common.MetricType{
		...
		if mtNotFound {
			scope.Infof("[NODE][%s][%s] No prediction metric %s found, send model jobs",
				dataGranularity, nodeName, metricType)
	...
	for _, lastPredictionMetric := range lastPredictionMetrics {
		if len(lastPredictionMetric.GetData()) == 0 {
			...
			if mID != "" && utils.IsModelExpired(
				...
				scope.Infof("[NODE][%s][%s] Send model job due to the model (id: %s, model max used times: %d, now: %d) of metric %s is expired",
					dataGranularity, nodeName, mID, modelMaxUsedTimes, time.Now().Unix(), lastPredictionMetric.GetMetricType().String())
				sender.sendJob(node, queueSender, pdUnit, granularity, lastPredictionMetric.GetMetricType())
				continue
			}
