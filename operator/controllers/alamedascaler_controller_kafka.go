...
func chooseTopic(
	majorTopic string, topicsWantToConsume, currentConsumeTopics []string) string {
	var empty struct{}
...
// listConsumerGroups returns []ConsumerGroup by AlamedaScaler.Spec.Kafka and current map recorded consumerGroup consuming topics.
func (r AlamedaScalerKafkaReconciler) listConsumerGroups(
	...
	for _, consumerGroupSpec := range consumerGroupSpecs {
		...
		if topic == "" {
			r.Logger.Infof("ConsumerGroup(%s) does not consumes Topic(%s).",
				consumerGroupSpec.Name, topic)
			continue
		}
		...
		if consumerGroupSpec.Resource.Kubernetes != nil {
			...
			if err != nil {
				return nil, errors.Wrapf(err,
					"get matched kubernetes resource failed: ConsumerGroup.Name: %s",
...
func (r AlamedaScalerKafkaReconciler) getFirstCreatedMatchedKubernetesMetadata(
	...
	r.Logger.Debugf(
		"List deploymentconfigs by matched namespace %s and labels %+v and get result %+v",
	...
	r.Logger.Debugf(
		"List statefulsets by matched namespace %s and labels %+v and get result %+v",
	...
	case earliestDeploymentCreationTimestamp.Time:
		...
		r.Logger.Debugf(
			"deployment create timestamp compared get replicas: %v",
		...
		for _, pod := range pods {
			...
			if err != nil {
				return consumerGroupEntity,
					errors.Wrap(err, "get volume capacity used by Pod failed")
			}
	...
	case earliestStatefulSetCreationTimestamp.Time:
		...
		consumerGroupEntity.ResourceMemoryRequest =
			strconv.FormatInt(resource.Requests.Memory().Value(), 10)
		r.Logger.Debugf(
			"statefulset create timestamp compared get replicas: %v",
			controller.Status.ReadyReplicas)
	...
	case earliestDeploymentConfigCreationTimestamp.Time:
		...
		consumerGroupEntity.ResourceMemoryRequest =
			strconv.FormatInt(resource.Requests.Memory().Value(), 10)
		r.Logger.Debugf(
			"deploymentconfig create timestamp compared get replicas: %v",
			controller.Status.ReadyReplicas)
		pods, err := resourcesLister.ListPodsByDeploymentConfig(
			controller.GetNamespace(), controller.GetName())
		if err != nil {
			return consumerGroupEntity, errors.Wrap(err, "list pods by Deployment failed")
		}
...
func (r AlamedaScalerKafkaReconciler) syncWithDatahub(
	...
	r.Logger.Debugf("Synchronize with Datahub. Topics: %+v, ConsumerGroups: %+v",
...
func (r AlamedaScalerKafkaReconciler) getTopicsToDelete(
	ctx context.Context, exporterNamespace string,
	alamedaScalerNamespace, alamedaScalerName string, topics []entities.ApplicationKafkaTopic) (
	[]entities.ApplicationKafkaTopic, error) {
	empty := struct{}{}
