...
import (
	...
	"github.com/containers-ai/alameda/operator/datahub/client/machineset/entity"
...
type MachineSetRepository struct {
	datahubClient datahub.DatahubServiceClient
	schemaConfig  config
	logger        *log.Scope
}

func NewMachineSetRepository(datahubClient datahub.DatahubServiceClient, logger *log.Scope) *MachineSetRepository {
	if logger == nil {
		logger = log.RegisterScope("datahub-client", "", 0)
	}
	return &MachineSetRepository{
...
func (n MachineSetRepository) CreateMachineSets(ctx context.Context, machineSets []machineset.MachineSet) error {
	if len(machineSets) == 0 {
		return nil
	}
	req, err := n.newWriteDataRequestByMachineSets(machineSets)
	if err != nil {
		return errors.Wrap(err, "new WriteDataRequest by machineSets failed")
	}
...
func (n MachineSetRepository) ListMachineSets(ctx context.Context, option ListMachineSetsOption) ([]machineset.MachineSet, error) {
	req, err := n.newReadDataRequestForMachineSets(option)
	if err != nil {
	...
	entities := make([]entity.MachineSet, 0)
	if err := decodeSlice(data, &entities); err != nil {
		return nil, errors.Wrap(err, "decode data failed")
	}
	machineSets := make([]machineset.MachineSet, 0, len(entities))
	for _, e := range entities {
		machineSets = append(machineSets, machineset.MachineSet{
...
func (n MachineSetRepository) DeleteMachineSets(ctx context.Context, machineSets []machineset.MachineSet) error {
	if len(machineSets) == 0 {
		return nil
	}
	req, err := n.newDeleteDataRequestByMachineSets(machineSets)
	if err != nil {
		return errors.Wrap(err, "new DeleteDataRequeset by machineSets failed")
	}
...
func (n MachineSetRepository) DeleteMachineSetsByOption(ctx context.Context, option DeleteMachineSetsOption) error {
	deleteData, err := newDeleteData(n.schemaConfig.machineSet.machineSet.delete.measurements[0], option)
	if err != nil {
		return errors.Wrap(err, "new DeleteData failed")
	}
	schemaMeta := n.schemaConfig.machineSet.machineSet.delete.schemaMeta
...
func (n MachineSetRepository) sendWriteDataRequest(ctx context.Context, req data.WriteDataRequest) error {
...
func (n MachineSetRepository) sendReadDataRequest(ctx context.Context, req data.ReadDataRequest) (data.Data, error) {
...
func (n MachineSetRepository) sendDeleteDataRequest(ctx context.Context, req data.DeleteDataRequest) error {
...
func (n MachineSetRepository) newWriteDataRequestByMachineSets(machineSets []machineset.MachineSet) (data.WriteDataRequest, error) {
	entities := make([]entity.MachineSet, 0, len(machineSets))
	for _, machineSet := range machineSets {
		entity := entity.NewMachineSet(machineSet)
		entities = append(entities, entity)
	}

	writeData, err := newWriteData(n.schemaConfig.machineSet.machineSet.create.measurements[0], entities)
	if err != nil {
		return data.WriteDataRequest{}, errors.Wrap(err, "new WriteData failed")
	}
	req := data.WriteDataRequest{
		SchemaMeta: &schemas.SchemaMeta{
			Scope:    schemas.Scope(schemas.Scope_value[n.schemaConfig.machineSet.machineSet.create.schemaMeta.scope]),
			Category: n.schemaConfig.machineSet.machineSet.create.schemaMeta.category,
			Type:     n.schemaConfig.machineSet.machineSet.create.schemaMeta.type_,
		},
...
func (n MachineSetRepository) newDeleteDataRequestByMachineSets(machineSets []machineset.MachineSet) (data.DeleteDataRequest, error) {
	entities := make([]entity.MachineSet, 0, len(machineSets))
	for _, machineSet := range machineSets {
		entity := entity.NewMachineSet(machineSet)
		entities = append(entities, entity)
	}
	deleteData, err := newDeleteData(n.schemaConfig.machineSet.machineSet.delete.measurements[0], entities)
	if err != nil {
		return data.DeleteDataRequest{}, errors.Wrap(err, "new DeleteData failed")
	}
	req := data.DeleteDataRequest{
		SchemaMeta: &schemas.SchemaMeta{
			Scope:    schemas.Scope(schemas.Scope_value[n.schemaConfig.machineSet.machineSet.delete.schemaMeta.scope]),
			Category: n.schemaConfig.machineSet.machineSet.delete.schemaMeta.category,
			Type:     n.schemaConfig.machineSet.machineSet.delete.schemaMeta.type_,
		},
...
func (n MachineSetRepository) newReadDataRequestForMachineSets(option ListMachineSetsOption) (data.ReadDataRequest, error) {
	condition, err := newCondition(option)
	if err != nil {
		return data.ReadDataRequest{}, errors.Wrap(err, "new condition failed")
	}
	readData := make([]*data.ReadData, 0, len(n.schemaConfig.machineSet.machineSet.list.measurements))
	for _, measurement := range n.schemaConfig.machineSet.machineSet.list.measurements {
	...
	req := data.ReadDataRequest{
		SchemaMeta: &schemas.SchemaMeta{
			Scope:    schemas.Scope(schemas.Scope_value[n.schemaConfig.machineSet.machineSet.list.schemaMeta.scope]),
			Category: n.schemaConfig.machineSet.machineSet.list.schemaMeta.category,
			Type:     n.schemaConfig.machineSet.machineSet.list.schemaMeta.type_,
		},
		ReadData: readData,
	}
	return req, nil
