...
func isScalerValid(client *client.Client, scalerObj *validatingObject) (bool, error) {
	...
	for _, scaler := range scalers {
		...
		for _, selectedDeployment := range selectedDeployments {
			if _, ok := scaler.Status.AlamedaController.Deployments[autoscalingv1alpha1.NamespacedName(fmt.Sprintf("%s/%s", selectedDeployment.GetNamespace(), selectedDeployment.GetName()))]; ok {
		...
		for _, selectedDeploymentConfig := range selectedDeploymentConfigs {
			if _, ok := scaler.Status.AlamedaController.DeploymentConfigs[autoscalingv1alpha1.NamespacedName(fmt.Sprintf("%s/%s", selectedDeploymentConfig.GetNamespace(), selectedDeploymentConfig.GetName()))]; ok {
